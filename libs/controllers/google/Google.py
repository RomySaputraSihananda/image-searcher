    ...
    async def __search_image_by_url(self, url_image: str = None) -> JSONResponse:
        try:
            ...
            if(response.status_code != 200): 
                return JSONResponse(content=BodyResponse(HTTPStatus.NOT_FOUND, message='url image not found').__dict__, status_code=HTTPStatus.NOT_FOUND)

            format: str = imghdr.what(None, b"".join(response.iter_content(chunk_size=128)))

            if(not format): 
                return JSONResponse(content=BodyResponse(HTTPStatus.BAD_REQUEST, message='url not contain image').__dict__, status_code=HTTPStatus.BAD_REQUEST)
